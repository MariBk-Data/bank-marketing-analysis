...
*   **Customer Segmentation** (Demographic & Credit Profiling)
*   **Conversion Rate Analysis** (Funnel Optimization)
*   **Cross-Factor & Synergy Analysis**
*   **Interactive Dashboard Development**
*   **Data Visualization** (Dual-Axis Combo Charts)
...
*   **Складні функції Google Sheets:** Автоматизація розрахунків та динамічний переклад даних за допомогою зв'язок функцій `BYROW`, `LAMBDA`, `UNIQUE`, `SORT`, `SWITCH` та `QUERY`, `Pivot Tables`, `Conditional Formatting`.

---

## 1.2. Загальний вигляд KPI-Дашборду проекту
Для комплексного моніторингу кампанії було розроблено єдину інтерактивну панель бізнес-індикаторів:

![KPI Дашборд маркетингової кампанії банку](dashboard.png)
![KPI Дашборд маркетингової кампанії банку_2](dashboard2.png)
...
* **Критичні точки:** Для молоді 32,22% та особливо для пенсіонерів старше 65 років іпотека є непосильним фінансовим тягарем. Конверсія літніх клієнтів з іпотекою є найнижчою серед усіх досліджених вікових груп і становить лише 4,27%, всього 17 успішних кейсів із 398.
...
*   **Парадокс категорії `success` (Лояльність понад усе):** Лише **26,42%** лояльних клієнтів мають іпотечне навантаження. Проте, коли банк контактує з цією закредитованою групою, рівень довіри повністю нівелює фактор боргу — конверсія досягає до абсолютного максимуму по банку — **87,99%**.
*   **Один із можливих пояснень низької конверсії сегмента `failure`:** Крос-аналіз показав, чому минулі маркетингові кампанії для цієї групи завершилися невдачею. Цей сегмент має найвищу закредитованість у банку — **59,77%** клієнтів обтяжені іпотекою. Оскільки їхнє фінансове становище не змінилося, повторні «холодні» дзвінки дають низьку конверсію (**39,24%**). Банк фактично витрачає ресурси на аудиторію, яка фізично не має вільних коштів.
*   **Низька ефективність роботи з холодною базою (`unknown`):** Майже половина нових клієнтів (**48,02%**) мають відкриту іпотеку. Спроби продати їм депозит без попередньої історії взаємодії демонструють найнижчу ефективність — всього **32,14%** конверсії при великих витратах часу операторів (3 998 дзвінків).

> 💡 
> *![Двопанельний комбінований графік](chart_combo.png)*
>  **Двопанельний комбінований графік (Dual-Axis Combo Chart)**. Стовпчики (ліва вісь) показують рівень закредитованості сегмента (пік на `failure` — 59%), а лінія тренду (права вісь) відображає дзеркальну конверсію в депозит (пік на `success` — 87%). Графік візуально доводить антикореляцію: що вища закредитованість групи, то гірше вона купує депозити (окрім суперлояльного сегмента).

---

## 4. Обмеження дослідження

- Категорія `unknown` у змінній `poutcome` охоплює 74,59% клієнтів.
- Аналіз є описовим і демонструє взаємозв'язки між факторами та конверсією, але не доводить причинно-наслідкових залежностей.
- Дані є історичними та не відображають поточну маркетингову політику банку.
- Рекомендації потребують подальшого тестування перед впровадженням у бізнес-процеси.

---

## 5. Загальний висновок для менеджменту банку
